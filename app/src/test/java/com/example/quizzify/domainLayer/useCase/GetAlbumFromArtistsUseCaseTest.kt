...
class GetAlbumFromArtistsUseCaseTest {
    @get:Rule
    val mainDispatcherRule = MainDispatcherRule()

    private val spotifyRepository: SpotifyRepository = mockk()
    private val getAlbumFromArtistsUseCase = GetAlbumFromArtistsUseCase(spotifyRepository)
    ...
    fun `invoke() should first emit Resource Loading`() = runTest{
        val artistIds = listOf("1", "2", "3")
        val response = Resource.Success(listOf<Album>())

        //Mock the response
        coEvery { spotifyRepository.albumArtists(artistIds) } returns response

        val actual = getAlbumFromArtistsUseCase(artistIds)
    ...
    fun `invoke() should emit Resource Success`() = runTest{
        ...
        val actual = getAlbumFromArtistsUseCase(artistIds).toList()
