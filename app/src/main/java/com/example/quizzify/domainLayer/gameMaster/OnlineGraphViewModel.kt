...
@HiltViewModel
class OnlineGraphViewModel @Inject constructor(
    private val getOnlineUseCase: GetOnlineUseCase,
    private val getRankingUseCase: GetRankingUseCase,
) : ViewModel() {

    private val _state = mutableStateOf(OnlineGraphState())
    val state: State<OnlineGraphState> = _state


    fun fetchGraph() {
        viewModelScope.launch {
            Log.d("RANKING", "Before Use CaseONLINE")
            getOnlineUseCase().collect { res ->
                when (res) {
                    is Resource.Success -> {
                        Log.d("RANKING", "SuccessONLINE")
                    ...
                    is Resource.Loading -> {
                        Log.d("RANKING", "LoadingONLINE")
                    }
                }
            }
            _state.value = _state.value.copy(
                _loading_graph = false
            )
    ...
    fun fetchRank() {
        viewModelScope.launch {
            getRankingUseCase("Online").collect { res ->
                when (res) {
                    is Resource.Success -> {
                        try {
                            _state.value = _state.value.copy(
                            ...
                            _state.value = _state.value.copy(
                                errorOccurred = true,
                                errorMessage = "Error in fetching ranking, please try again later."
                            )
                        }
                    }
                    is Resource.Error -> {
